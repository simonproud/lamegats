...
class CrmToAts implements ICrmToAts, IFromCrm
{
    ...
    {
        $response = json_decode($this->client->request($method, $this->config['base_uri'], ['form_params'=> array_merge($params, ['cmd' => 'accounts', 'token' => $this->config['token']])])->getBody(), true);
        $accounts = collect();
    ...
    {
        return $this->client->request($method, $this->config['base_uri'], ['form_params'=> array_merge($params, ['cmd' => 'makeCall', 'token' => $this->config['token']])]);
    }
